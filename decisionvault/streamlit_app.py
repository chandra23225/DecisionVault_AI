...
from decisionvault.config import is_placeholder_config_value
...
if not API_KEY or is_placeholder_config_value(API_KEY):
    st.error("GEMINI_API_KEY is missing or still a placeholder. Please add a real key to your .env file.")
    st.stop()
    ...
    if generate_clicked:
        with st.spinner("Extracting decision records using Gemini..."):
            try:
                ...
                st.error("Gemini could not process the request right now.")
                st.write(str(e))
                st.caption("Check your API key, network connection, and Gemini quota status.")
            ...
            if not user_question.strip():
                ...
                with st.spinner("Searching decision memory..."):
                    try:
                        ...
                        st.error("Gemini could not answer the question right now.")
                        st.write(str(e))
                        st.caption("Check your API key, network connection, and Gemini quota status.")
                        st.stop()
